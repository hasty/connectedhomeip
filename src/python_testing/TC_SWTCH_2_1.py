...
class SWTCH_2_1(MatterBaseTest):

    def desc_SWTCH_2_1(self) -> str:
    ...
    def steps_SWTCH_2_1(self) -> list[TestStep]:
        steps = [
            TestStep("1", "Read NumberOfPositions attribute"),
    ...
    async def test_SWTCH_2_1(self):
        ...
        self.step("1")
        self.NumberOfPositions = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.NumberOfPositions)
        matter_asserts.assert_valid_uint8(self.NumberOfPositions, 'NumberOfPositions')
        asserts.assert_greater_equal(self.NumberOfPositions, 2)

        self.step("2")
        val = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.CurrentPosition)
        matter_asserts.assert_valid_uint8(val, 'CurrentPosition')
