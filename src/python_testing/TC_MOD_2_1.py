...
class MOD_2_1(MatterBaseTest):

    def desc_MOD_2_1(self) -> str:
        """Returns a description of this test"""
        return "Attributes with Server as DUT"

    def pics_MOD_2_1(self) -> list[str]:
        """This function returns a list of PICS for this test case that must be True for the test to be run"""
        return ["MOD.S"]

    def steps_MOD_2_1(self) -> list[TestStep]:
        steps = [
            TestStep("1", "Read Description attribute"),
            TestStep("2", "Read StandardNamespace attribute"),
            TestStep("3", "Read SupportedModes attribute"),
            TestStep("4", "Read CurrentMode attribute"),
    ...
    @run_if_endpoint_matches(has_cluster(Clusters.ModeSelect))
    async def test_MOD_2_1(self):
        ...
        asserts.assert_less_equal(len(val), 64, "Description must have length at most 64!")

        self.step("2")
        val = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.StandardNamespace)
        if val is not NullValue:
            matter_asserts.assert_valid_uint16(val, 'StandardNamespace must be uint16')
        ...
        self.step("5")
        if await self.attribute_guard(endpoint=endpoint, attribute=attributes.StartUpMode):
            val = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.StartUpMode)
    ...
    async def test_checkModeOptionStruct(self, 
                                 ...
                                 struct: Clusters.ModeSelect.Structs.ModeOptionStruct = None):
