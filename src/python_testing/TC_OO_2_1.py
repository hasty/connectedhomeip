...
cluster = Clusters.On/Off

class OO_2_1(MatterBaseTest):

    def desc_OO_2_1(self) -> str:
        """Returns a description of this test"""
        return "Attributes with Server as DUT"

    def pics_OO_2_1(self) -> list[str]:
        """This function returns a list of PICS for this test case that must be True for the test to be run"""
        return ["OO.S"]

    def steps_OO_2_1(self) -> list[TestStep]:
        steps = [
            TestStep("1", "Read OnOff attribute"),
            TestStep("2", "Read GlobalSceneControl attribute"),
            TestStep("3", "Read OnTime attribute"),
            TestStep("4", "Read OffWaitTime attribute"),
            TestStep("5", "Read StartUpOnOff attribute"),
        ]
        return steps

    @run_if_endpoint_matches(has_cluster(Clusters.On/Off))
    async def test_OO_2_1(self):
        endpoint = self.get_endpoint()
        attributes = cluster.Attributes

        self.step("1")
        val = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.OnOff)
