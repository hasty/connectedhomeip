...
cluster = Clusters.AirQuality

class AIRQUAL_2_1(MatterBaseTest):

    def desc_AIRQUAL_2_1(self) -> str:
        """Returns a description of this test"""
        return "Attributes with Server as DUT"

    def pics_AIRQUAL_2_1(self) -> list[str]:
        """This function returns a list of PICS for this test case that must be True for the test to be run"""
        return ["AIRQUAL.S"]

    def steps_AIRQUAL_2_1(self) -> list[TestStep]:
        steps = [
            TestStep("1", "Read AirQuality attribute"),
        ]
        return steps

    @run_if_endpoint_matches(has_cluster(Clusters.AirQuality))
    async def test_AIRQUAL_2_1(self):
