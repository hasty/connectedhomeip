...
cluster = Clusters.IlluminanceMeasurement

class ILL_2_1(MatterBaseTest):

    def desc_ILL_2_1(self) -> str:
        """Returns a description of this test"""
        return "Attributes with Server as DUT"

    def pics_ILL_2_1(self) -> list[str]:
        """This function returns a list of PICS for this test case that must be True for the test to be run"""
        return ["ILL.S"]

    def steps_ILL_2_1(self) -> list[TestStep]:
        steps = [
            ...
            TestStep("5", "Read LightSensorType attribute"),
        ]
        return steps
    ...
    @run_if_endpoint_matches(has_cluster(Clusters.IlluminanceMeasurement))
    async def test_ILL_2_1(self):
        ...
        if val is not NullValue:
            matter_asserts.assert_valid_uint16(val, 'MeasuredValue')
        ...
        if self.MinMeasuredValue is not NullValue:
            matter_asserts.assert_valid_uint16(self.MinMeasuredValue, 'MinMeasuredValue')
            asserts.assert_greater_equal(self.MinMeasuredValue, 1)
            asserts.assert_less_equal(self.MinMeasuredValue, 65533)
        ...
        if self.MaxMeasuredValue is not NullValue:
            matter_asserts.assert_valid_uint16(self.MaxMeasuredValue, 'MaxMeasuredValue')
